...
$Id: gdisp_sampledSymbols.c,v 1.1 2004/03/26 21:09:17 esteban Exp $
				...
				GtkWidget *parent )
{
  ...
  while (providerItem != (GList*)NULL) {
     ...
     * Set the style of the frame.
     */
    sprintf(rowBuffer," %d : %s ",providerCount,provider->pName->str);
    ...
    for (pSampleCpt=0; pSampleCpt<pSampleList->len; pSampleCpt++) {
      ...
      /* FIXME : how to get in touch with the symbol ??? */

      notAvailable = gtk_label_new("Not available...");

      gtk_container_add(GTK_CONTAINER(sFrame),
			notAvailable);

      gtk_widget_show(notAvailable);
