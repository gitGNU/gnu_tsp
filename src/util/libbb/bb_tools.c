...
$Header: /sources/tsp/tsp/src/util/libbb/bb_tools.c,v 1.36 2007/09/04 23:37:20 deweerdt Exp $
...
E_BBTOOLS_CMD_T bbtools_checkargs(bbtools_request_t * req)
{
...
int32_t bbtools(bbtools_request_t * req)
{
	...
	if (!((E_BBTOOLS_UNKNOWN == req->cmd) ||
	      (E_BBTOOLS_GENERIC == req->cmd) || (E_BBTOOLS_HELP == req->cmd) || (E_BBTOOLS_CHECK_ID == req->cmd) || (E_BBTOOLS_LOAD == req->cmd) || (E_BBTOOLS_CREATE == req->cmd))
		...
		if (NULL == req->theBB) {
			if (!req->silent) {
				bbtools_logMsg(req->stream, "Blackboard <%s> does not exist\n", req->argv[0]);
			}
			return -1;
		} else {	/* assign bbname */
...
void bbtools_usage(bbtools_request_t * req)
{
	...
	switch (req->cmd) {
	case E_BBTOOLS_UNKNOWN:
		fprintf(req->stream, "%s::unknown %s command\n", bbtools_cmdname_tab[E_BBTOOLS_GENERIC], bbtools_cmdname_tab[E_BBTOOLS_GENERIC]);
	case E_BBTOOLS_GENERIC:
	case E_BBTOOLS_HELP:
		fprintf(req->stream, "TSP bb_tools (BB vId=0x%08X, TSP v%s, %s)\n", BB_VERSION_ID, TSP_SOURCE_VERSION, TSP_PROJECT_URL);
		fprintf(req->stream, "Usage: %s [bb_tools_opts] <bb_tools_cmd> [cmd_opts]\n", bbtools_cmdname_tab[E_BBTOOLS_GENERIC]);
		...
		for (i = E_BBTOOLS_GENERIC + 1; i < E_BBTOOLS_LASTCMD; ++i) {
			fprintf(req->stream, "    %s%s: %s\n", bbtools_cmdname_tab[i], bbtools_fillspace(20, bbtools_cmdname_tab[i]), bbtools_cmdhelp_tab[i]
			    );
		}
		break;
	case E_BBTOOLS_READ:
		fprintf(req->stream, "Usage: %s <bbname> <symbol_name>\n", bbtools_cmdname_tab[E_BBTOOLS_READ]);
		break;
	case E_BBTOOLS_WRITE:
		fprintf(req->stream, "Usage: %s <bbname> <symbol_name> <value>\n", bbtools_cmdname_tab[E_BBTOOLS_WRITE]);
		break;
	case E_BBTOOLS_DUMP:
	...
	case E_BBTOOLS_FIND:
		...
		break;
	case E_BBTOOLS_CHECK_ID:
		fprintf(req->stream, "Usage: %s <bbname> [<user_specific_value>]\n", bbtools_cmdname_tab[E_BBTOOLS_CHECK_ID]);
		break;
	case E_BBTOOLS_DESTROY:
	...
	case E_BBTOOLS_CHECK_VERSION:
		...
		break;
	default:
		fprintf(req->stream, "default: should never be reached?\n");
	}

	if ((req->cmd >= E_BBTOOLS_UNKNOWN) && (req->cmd < E_BBTOOLS_LASTCMD)) {
		fprintf(req->stream, "%s::%s\n", bbtools_cmdname_tab[req->cmd], bbtools_cmdhelp_tab[req->cmd]);
	}

}/* end of bb_tools_usage */


int32_t bbtools_unimplemented_cmd(const char *cmdname)
{
...
int32_t bbtools_read(bbtools_request_t * req)
{
	int32_t retval = 0;
...
#define MSG_SIZE 1024
	...
	if (bb_utils_parsearrayname(req->argv[1], name, bb_varname_max_len(), array_index, &array_index_len)) {
		bbtools_logMsg(req->stream, "%s: cannot parse symname <%s>", bbtools_cmdname_tab[E_BBTOOLS_READ], req->argv[1]);
		retval = -1;
	} else {
		bb_set_varname(&sym_data_desc, name);
		if (req->verbose) {
			char *n = bb_get_varname(&sym_data_desc);
			bbtools_logMsg(req->stream, "%s: Trying to read symbol <%s> on blackboard <%s>...\n", bbtools_cmdname_tab[E_BBTOOLS_READ], n, req->bbname);
			free(n);
			nbcar = 0;
			...
			nbcar += snprintf(&msg[nbcar], MSG_SIZE - nbcar, " %s", "]");
			msg[nbcar] = '\0';
			bbtools_logMsg(req->stream, "%s: array_index_len = <%d> indexes = %s...\n", bbtools_cmdname_tab[E_BBTOOLS_READ], array_index_len, msg);
	...
	if (NULL == sym_value) {
		...
		if ((array_index_len > 0) && (j == 0)) {
			...
			bbtools_logMsg(req->stream, "%s: index <%d> exceeding symbol array dimension <%d>\n",
				       bbtools_cmdname_tab[E_BBTOOLS_READ], array_index[array_index_len - 1], aliasstack[j - 1].dimension);
		} else {
			if (array_index_len > 0) {
			...
			if (req->verbose) {
				...
				if ((E_BB_CHAR == sym_data_desc.type) || (E_BB_UCHAR == sym_data_desc.type)) {
...
int32_t bbtools_write(bbtools_request_t * req)
{
	int32_t retval = 0;
	S_BB_DATADESC_T sym_data_desc;
	int32_t array_index[MAX_ALIAS_LEVEL];
	...
	if (req->argc < 3) {
		bbtools_logMsg(req->stream, "%s: <%d> argument(s) missing\n", bbtools_cmdname_tab[E_BBTOOLS_WRITE], 3 - req->argc);
		bbtools_usage(req);
		retval = -1;
		return retval;
	}
	...
	memset(&sym_data_desc, 0, sizeof(S_BB_DATADESC_T));
	if (bb_utils_parsearrayname(req->argv[1], name, bb_varname_max_len(), array_index, &array_index_len)) {
		bbtools_logMsg(req->stream, "%s: cannot parse symname <%s>", bbtools_cmdname_tab[E_BBTOOLS_WRITE], req->argv[1]);
		retval = -1;
	} else {
		bb_set_varname(&sym_data_desc, name);
		if (req->verbose) {
			char *n = bb_get_varname(&sym_data_desc);
			bbtools_logMsg(req->stream, "%s: Trying to write symbol <%s> on blackboard <%s>...\n", bbtools_cmdname_tab[E_BBTOOLS_WRITE], n, req->bbname);
			free(n);
			nbcar = 0;
			nbcar += snprintf(&msg[nbcar], MSG_SIZE - nbcar, "%s", "[");
			for (i = 0; i < array_index_len; ++i) {
			...
			nbcar += snprintf(&msg[nbcar], MSG_SIZE - nbcar, " %s", "]");
			msg[nbcar] = '\0';
			bbtools_logMsg(req->stream, "%s: array_index_len = <%d> indexes = %s...\n", bbtools_cmdname_tab[E_BBTOOLS_WRITE], array_index_len, msg);
	...
	if (NULL == sym_value) {
		...
		if ((array_index_len > 0) && (j == 0)) {
			...
			bbtools_logMsg(req->stream, "%s: index <%d> exceeds symbol array dimension <%d> whose valid index range is <0..%d>\n",
				       bbtools_cmdname_tab[E_BBTOOLS_WRITE], array_index[array_index_len - 1], aliasstack[j - 1].dimension, aliasstack[j - 1].dimension - 1);
		} else {
			if (array_index_len > 0) {
				idx = array_index[array_index_len - 1];
			} else {
...
#ifdef XML2_FOUND
int32_t bbtools_load(bbtools_request_t * req)
{
	int32_t retcode = 0;
	int nr_args = 2;
	if (req->argc < nr_args) {
		bbtools_logMsg(req->stream, "%s: <%d> argument missing\n", bbtools_cmdname_tab[E_BBTOOLS_LOAD], nr_args - req->argc);
		bbtools_usage(req);
		retcode = -1;
		return retcode;
	}
	if (req->verbose) {
		bbtools_logMsg(req->stream, "%s: load BB <%s>\n", bbtools_cmdname_tab[E_BBTOOLS_LOAD], req->bbname);
	}

	req->bbname = strdup(req->argv[0]);
	bb_xml_load_file(req);
	free(req->bbname);
	return retcode;
}
#else
int32_t bbtools_load(bbtools_request_t * req)
{
	bbtools_logMsg(req->stream, "bb_load needs xml support, please recompile tsp with libxml2\n", bbtools_cmdname_tab[E_BBTOOLS_LOAD], req->bbname);
	return -1;
}
#endif /* XML2_FOUND */


int32_t bbtools_dump(bbtools_request_t * req)
{
	int32_t retcode = 0;
...
int32_t bbtools_find(bbtools_request_t * req)
{
	int32_t retcode = 0;
	char *varmatch;
	int32_t i;
	...
	if (req->argc < 2) {
		bbtools_logMsg(req->stream, "%s: <%d> argument missing\n", bbtools_cmdname_tab[E_BBTOOLS_FIND], 1 - req->argc);
		bbtools_usage(req);
		retcode = -1;
		return retcode;
	}
	...
	if (req->verbose) {
		bbtools_logMsg(req->stream, "%s: find symbol matching <%s> in  BB <%s>\n", bbtools_cmdname_tab[E_BBTOOLS_FIND], varmatch, req->bbname);
	}
	...
	if (req->verbose) {
		bbtools_logMsg(req->stream, "%s: found <%d> symbol(s) matching <%s> in  BB <%s>\n", bbtools_cmdname_tab[E_BBTOOLS_FIND], nmatch, varmatch, req->bbname);
	}
	free(varmatch);
	return retcode;
}/* end of bbtools_find */

int32_t bbtools_check_id(bbtools_request_t * req)
{
	int32_t retcode = 0;
...
int32_t bbtools_destroy(bbtools_request_t * req)
{
	int32_t retcode = 0;
	if (req->argc < 1) {
		bbtools_logMsg(req->stream, "%s: <%d> argument missing\n", bbtools_cmdname_tab[E_BBTOOLS_DESTROY], 1 - req->argc);
		bbtools_usage(req);
		retcode = -1;
		return retcode;
	}
	if (req->verbose) {
		bbtools_logMsg(req->stream, "%s: destroying BB <%s>\n", bbtools_cmdname_tab[E_BBTOOLS_DESTROY], req->bbname);
	}
	retcode = bb_destroy(&(req->theBB));
	return retcode;
}/* end of bbtools_destroy */

int32_t bbtools_create(bbtools_request_t * req)
{
	int32_t retcode = 0;
	uint32_t ndata;
	uint32_t datasize;
	...
	if (req->argc < 2) {
		bbtools_logMsg(req->stream, "%s: at least <%d> argument(s) missing\n", bbtools_cmdname_tab[E_BBTOOLS_CREATE], 2 - req->argc);
		bbtools_usage(req);
		retcode = -1;
		return retcode;
	}
	...
	if (req->verbose) {
		bbtools_logMsg(req->stream, "%s: creating BB <%s> with <%d> elements and <%d> bytes for datazone\n", bbtools_cmdname_tab[E_BBTOOLS_CREATE], req->bbname, ndata, datasize);
	}

	retcode = bb_create(&(req->theBB), req->bbname, ndata, datasize);
	if ((retcode != BB_OK) && (req->verbose)) {
		bbtools_logMsg(req->stream, "%s: creating failed to create BB <%s>\n", bbtools_cmdname_tab[E_BBTOOLS_CREATE], req->bbname);
	}
	return retcode;
}/* end of bbtools_create */

int32_t bbtools_publish(bbtools_request_t * req)
{
	int32_t retcode = 0;
	...
	if (req->argc < 2) {
		bbtools_logMsg(req->stream, "%s: at least <%d> argument(s) missing\n", bbtools_cmdname_tab[E_BBTOOLS_PUBLISH], 2 - req->argc);
		bbtools_usage(req);
		retcode = -1;
		return retcode;
	}
	...
	if (req->verbose) {
		...
		bbtools_logMsg(req->stream, "%s: publish symbol <%s> of type <%s> in BB <%s>\n", bbtools_cmdname_tab[E_BBTOOLS_PUBLISH], n, symbol_type_str, req->bbname);
		free(n);
	}
	...
	if (symbol_desc.type != 0) {
		...
		bbtools_logMsg(req->stream, "%s: publish FAILED unrecognized bb_type: <%s>\n", bbtools_cmdname_tab[E_BBTOOLS_PUBLISH], symbol_type_str);
	}

	return retcode;
}/* end of bbtools_publish */

int32_t bbtools_synchro_send(bbtools_request_t * req)
{
	int32_t retcode = 0;
	if (req->argc < 1) {
		bbtools_logMsg(req->stream, "%s: <%d> argument missing\n", bbtools_cmdname_tab[E_BBTOOLS_SYNCHRO_SEND], 1 - req->argc);
		bbtools_usage(req);
		retcode = -1;
		return retcode;
	}
	if (req->verbose) {
		bbtools_logMsg(req->stream, "%s: sending synchro to BB <%s>\n", bbtools_cmdname_tab[E_BBTOOLS_SYNCHRO_SEND], req->bbname);
	}
...
int32_t bbtools_synchro_recv(bbtools_request_t * req)
{
	int32_t retcode = 0;
	if (req->argc < 1) {
		bbtools_logMsg(req->stream, "%s: <%d> argument missing\n", bbtools_cmdname_tab[E_BBTOOLS_SYNCHRO_SEND], 1 - req->argc);
...
int32_t bbtools_memset(bbtools_request_t * req)
{
	int32_t retcode = 0;
	int32_t hexval;
	char value;
	...
	if (req->argc < 2) {
		bbtools_logMsg(req->stream, "%s: <%d> argument missing\n", bbtools_cmdname_tab[E_BBTOOLS_MEMSET], 1 - req->argc);
		bbtools_usage(req);
		retcode = -1;
		return retcode;
	}
	...
	if ((NULL != strstr(req->argv[1], "0x")) | (NULL != strstr(req->argv[1], "0X"))
	    ) {
		hexval = 1;
	} else {
		hexval = 0;
	...
	if (req->verbose) {
		bbtools_logMsg(req->stream, "%s: setting all data zone of BB <%s> to <0x%02x>\n", bbtools_cmdname_tab[E_BBTOOLS_MEMSET], req->bbname, value, value);
	}
	retcode = bb_data_memset(req->theBB, value);
	return retcode;
}/* end of bbtools_memset */

int32_t bbtools_check_version(bbtools_request_t * req)
{
	int32_t retcode = 0;
	if (req->argc < 1) {
		bbtools_logMsg(req->stream, "%s: <%d> argument missing\n", bbtools_cmdname_tab[E_BBTOOLS_CHECK_VERSION], 1 - req->argc);
		bbtools_usage(req);
		retcode = -1;
		return retcode;
	}
	if (req->verbose) {
		bbtools_logMsg(req->stream, "%s: checking BB version for BB <%s>\n", bbtools_cmdname_tab[E_BBTOOLS_CHECK_VERSION], req->bbname);
	...
	if (retcode != 0) {
		bbtools_logMsg(req->stream,
			       ...
			       bbtools_cmdname_tab[E_BBTOOLS_CHECK_VERSION], BB_VERSION_ID, req->theBB->bb_version_id);
	} else {
		bbtools_logMsg(req->stream,
			       "%s: OK.\nbb_tools and accessed BB [%s] version are the same <0x%08X>\n", bbtools_cmdname_tab[E_BBTOOLS_CHECK_VERSION], req->bbname, req->theBB->bb_version_id);
	}
	return retcode;
